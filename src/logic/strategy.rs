...
pub fn decide(game_state: GameState) -> Vec<PlayerAction> {
    ...
    for base in own_bases {
        ...
        for opponent in opponent_bases.clone() {
            ...
            if opponent.uid == 0 && base.population > game_state.config.base_levels[base.level as usize].max_population * 3 / 4 {req = game_state.config.base_levels[base.level as usize].spawn_rate}
