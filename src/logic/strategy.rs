...
pub fn decide(game_state: GameState) -> Vec<PlayerAction> {
    ...
    for base in own_bases {
        ...
        for opponent in opponent_bases.clone() {
            ...
            if opponent.uid == 0 && opponent.population > 3 && base.population > game_state.config.base_levels[base.level as usize].max_population * 3 / 4 {req = game_state.config.base_levels[base.level as usize].spawn_rate}
            if req < base.population && base.population_in_n_ticks(base.distance_to(&opponent), &game_state.config, &game_state.actions) > 0 {
        ...
        if let Some(target) = target {
            if base.population_in_n_ticks(base.distance_to(&target.0), &game_state.config, &game_state.actions) > target.1 + 2 {
